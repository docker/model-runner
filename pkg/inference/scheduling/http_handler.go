...
func (h *HTTPHandler) Configure(w http.ResponseWriter, r *http.Request) {
	...
	go func() {
		...
		if err != nil {
			h.scheduler.log.Warn(fmt.Sprintf("failed to marshal preload request body: %v", err))
			return
		}
		...
		if err != nil {
			h.scheduler.log.Warn(fmt.Sprintf("failed to create preload request: %v", err))
			return
		}
		...
		if recorder.Code != http.StatusOK {
			h.scheduler.log.Warn(fmt.Sprintf("background model preload failed with status %d: %s", recorder.Code, recorder.Body.String()))
		}
	}()
