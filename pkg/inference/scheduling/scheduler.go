...
func (s *Scheduler) handleOpenAIInference(w http.ResponseWriter, r *http.Request) {
	...
	// Compute the model ID that will be used to key the runner. For passthrough
	// backends, we'll just use a dummy model ID (though we'll still record the
	// real model name below) so that we don't need to allocate multiple runners
	// (one for each model used by a passthrough backend).
	var modelID string
