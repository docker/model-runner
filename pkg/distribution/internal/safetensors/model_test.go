...
func TestNewModel_EmptyPaths(t *testing.T) {
	_, err := NewModel([]string{})
	if err == nil {
		t.Error("NewModel() with empty paths should return error")
	}
}

func TestNewModel_InvalidFile(t *testing.T) {
	_, err := NewModel([]string{"/path/that/does/not/exist.safetensors"})
	if err == nil {
		t.Error("NewModel() with non-existent file should return error")
	}
}

func TestNewModel_ShardedModel(t *testing.T) {
	// Create multiple sharded safetensors files
	tmpDir := t.TempDir()

	// Create shard 1
	createTestShard(t, tmpDir, "model-00001-of-00002.safetensors", map[string]interface{}{
		"shard1.weight": map[string]interface{}{
			"dtype":        "F16",
			"shape":        []interface{}{float64(1000), float64(1000)},
			"data_offsets": []interface{}{float64(0), float64(2000000)},
		},
	})

	// Create shard 2
	createTestShard(t, tmpDir, "model-00002-of-00002.safetensors", map[string]interface{}{
		"shard2.weight": map[string]interface{}{
			"dtype":        "F16",
			"shape":        []interface{}{float64(1000), float64(1000)},
			"data_offsets": []interface{}{float64(0), float64(2000000)},
		},
	...
	// Create model with just the first shard path (should auto-discover)
	firstShardPath := filepath.Join(tmpDir, "model-00001-of-00002.safetensors")
	model, err := NewModel([]string{firstShardPath})
	if err != nil {
		t.Fatalf("NewModel() error = %v", err)
	}

	// Verify it has 2 layers (one per shard)
	...
	if len(layers) != 2 {
		t.Errorf("Layers() returned %d layers, want 2", len(layers))
	}

	// Verify config uses metadata from first shard
	config, err := model.Config()
	if err != nil {
		t.Fatalf("Config() error = %v", err)
	}

	// Parameters should be from first shard only (1000*1000 = 1000000)
	expectedParams := "1.00 M"
	if config.Parameters != expectedParams {
		t.Errorf("Config.Parameters = %v, want %v", config.Parameters, expectedParams)
	}
}

func createTestShard(t *testing.T, dir, filename string, header map[string]interface{}) {
	t.Helper()
