...
func (h *SchedulerMetricsHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		h.log.Error(fmt.Sprintf("Failed to get llama.cpp socket: %v", err))
	...
	if err != nil {
		h.log.Error(fmt.Sprintf("Failed to create metrics request: %v", err))
	...
	if err != nil {
		h.log.Error(fmt.Sprintf("Failed to fetch metrics from backend: %v", err))
	...
	if _, err := io.Copy(w, resp.Body); err != nil {
		h.log.Error(fmt.Sprintf("Failed to copy metrics response: %v", err))
		return
	}
